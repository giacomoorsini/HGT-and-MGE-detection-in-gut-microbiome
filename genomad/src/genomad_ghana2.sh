...
# Checkpoint
if [ ! -f "${GH2_DIR}/${SAMPLE}.filtered_contigs.fa.bz2" ]; then
        echo -e "\t\t Unable to locate the contig file in original directory. Aborting."
        exit 1
fi

# Start waafle_search analysis
genomad end-to-end --cleanup --splits 8 "${GH2_DIR}/${SAMPLE}.filtered_contigs.fa.bz2" $OUT $DB
