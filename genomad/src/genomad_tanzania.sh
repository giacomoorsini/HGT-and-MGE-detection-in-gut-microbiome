...
OUT="${HOMEDIR}/output/CM_tanzania"
...
# Checkpoint
if [ ! -f "${TZ_DIR}/${SAMPLE}/${SAMPLE}.spades.fasta.bz2" ]; then
        echo -e "\t\t Unable to locate the contig file in original directory. Aborting."
        exit 1
fi

# Start waafle_search analysis
genomad end-to-end --cleanup --splits 8 "${TZ_DIR}/${SAMPLE}/${SAMPLE}.spades.fasta.bz2" $OUT $DB
