...
# Checkpoint
if [ ! -f $TMP/$SAMPLE".spades.fasta" ]; then
...
waafle_search --threads 64 --out $SEARCH/"${SAMPLE}_search.blastout" $TMP/$SAMPLE".spades.fasta" $DB
...
waafle_orgscorer --outdir $ORGSCORE $TMP/$SAMPLE".spades.fasta" $SEARCH/"${SAMPLE}_search.blastout" $GENECALL/"${SAMPLE}_genecall.gff" $TAX
